...
class Wmgp::Renderers::MatchResultCardTest < ActiveSupport::TestCase
  test "versus header uses VS label" do
    match = create_match_for_renderer_test!(home_name: "Alpha Team", away_name: "Beta Team")

    html = Wmgp::Renderers::MatchResultCard.new(match).send(:versus_html)
  ...
  test "renderer falls back to board scores when stored winners are stale" do
    ...
    renderer = Wmgp::Renderers::MatchResultCard.new(match.reload)
  ...
  test "versus header embeds team icon as data uri when attached" do
    ...
    html = Wmgp::Renderers::MatchResultCard.new(match.reload).send(:versus_html)

...
PoC skeleton で採用したディレクトリ:

- `app/models` — Core Domain
- `apps/ledger/app/services/rule_modules/` — Core 公開 IF (`Base` 抽象 + `Registry`)
- `apps/ledger/app/services/standings/` / `match_exports/` / `standings_exports/` — 既存実装 (= 後続 Issue で各 league module 配下に移植する切り出し対象)
- `apps/ledger/app/leagues/<key>/` — 各 league module の入れ物。 `app/leagues` 直下が zeitwerk autoload root なので、 ファイル名と class 名は素直な対応 (= `app/leagues/wmgp/league_module.rb` → `Wmgp::LeagueModule`)
- `apps/ledger/config/initializers/rule_modules.rb` — boot 時に各 league module を `RuleModules::Registry` に登録

重要なのは名前よりも、 `<league key> 固有ルール` と `使い回すコア` を混ぜないこと。

## 別 league module を追加する手順

新しい league (例: `examplecup`) を載せる時:

1. **module 入れ物作成**
   - `apps/ledger/app/leagues/examplecup/league_module.rb` を作成し、 `Examplecup::LeagueModule < ::RuleModules::Base` を実装
   - 必要に応じて `rules/` / `renderers/` / `reports/` サブディレクトリを切る
   - `apps/ledger/app/leagues/examplecup/README.md` で構造説明
2. **boot 時登録**
   - `apps/ledger/config/initializers/rule_modules.rb` に `RuleModules::Registry.register(Examplecup::LeagueModule.new)` を追加
3. **ruleset 定義** (順位ルール / フォーマット等の JSON 定義)
   - `apps/ledger/config/rulesets/examplecup.json` を作成
4. **テスト**
   - `apps/ledger/test/services/rule_modules/registry_test.rb` に `examplecup` の登録確認を追加
   - 各実装には個別の unit test を追加

最小サンプルは `apps/ledger/app/leagues/wmgp/` を参照。

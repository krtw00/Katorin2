...
このドキュメントは、Katorin2プロジェクトのドキュメント作成における方針を定める。
...
**良い例:**
> 大会は「タイトル」「説明」「ステータス」「形式」「参加上限」を持つ。
> ステータスは draft → published → recruiting → in_progress → completed の順で遷移する。

**避けるべき例:**
50行以上のインターフェース定義やスキーマ定義を記載すること。
...
| インターフェース（画面設計） | 目的 → 背景 → 画面一覧 → 関連 |
| 手順（setup, contributing） | 目的 → 前提条件 → 手順 → 関連 |
...
| 表記 | 意味 |
|------|------|
| @02-architecture/directory-structure.md | docs/からの相対パス |
...
| 記述場所 | @リンク | 実際のパス |
|---------|--------|-----------|
| 01-introduction/tech-stack.md | @04-data/database-design.md | docs/04-data/database-design.md |
| 05-features/requirements.md | @appendix/glossary.md | docs/appendix/glossary.md |

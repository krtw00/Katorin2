...
docker exec -i "$POSTGRES_CONTAINER" psql -U postgres -d postgres -tAc \
  "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${STAGING_DB}' AND pid <> pg_backend_pid();" \
  >/dev/null

log "dumping ${PROD_DB} -> ${DUMP_FILE} (inside container) and restoring into ${STAGING_DB}..."
docker exec -i "$POSTGRES_CONTAINER" bash <<EOF
set -euo pipefail
pg_dump -U postgres --format=custom --no-owner --no-privileges ${PROD_DB} > ${DUMP_FILE}
psql -U postgres -d postgres -v ON_ERROR_STOP=1 -c 'DROP DATABASE IF EXISTS "${STAGING_DB}";'
psql -U postgres -d postgres -v ON_ERROR_STOP=1 -c 'CREATE DATABASE "${STAGING_DB}" OWNER "${STAGING_DB_USER}";'
pg_restore -U postgres --no-owner --no-acl --role="${STAGING_DB_USER}" -d "${STAGING_DB}" "${DUMP_FILE}"
rm -f ${DUMP_FILE}
EOF

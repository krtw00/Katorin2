...
2. src/types/round.ts を新規作成し、以下をコピー:
   ...
   - Tournament → Round に型名変更（各カラム名も: league_id→league_id, format→format, round_order→round_order）
...
src/components/leagues/ ディレクトリを src/components/league/ にリネームしてください。

手順:
1. src/components/league/ ディレクトリを作成
2. src/components/leagues/ 内の全ファイルを src/components/league/ にコピー
3. src/components/leagues/ 内のファイルを削除
4. src/components/league/ 内の各ファイルで:
   ...
   - Supabaseクエリの league_id → league_id に変更
...
src/app/[locale]/(main)/leagues/ を src/app/[locale]/(main)/leagues/ にリネームしてください。

手順:
1. src/app/[locale]/(main)/leagues/ ディレクトリを作成（サブディレクトリ含む再帰的に）
2. src/app/[locale]/(main)/leagues/ 内の全ファイルを対応する場所にコピー
3. src/app/[locale]/(main)/leagues/ を削除
4. 移動した全ファイル内で:
   - import の "@/components/leagues/" → "@/components/league/" に変更
   - import の "@/types/series" → "@/types/league" に変更
   - Supabaseクエリの .from("series") → .from("leagues") に変更
   - Supabaseクエリの .from("tournaments") → .from("rounds") に変更
   - カラム参照の league_id → league_id に変更
   - カラム参照の round_id → round_id に変更
   - league_config → league_config に変更
   - href="/leagues/" → href="/leagues/" に変更
...
1. import パスの更新:
   - "@/components/leagues/" → "@/components/league/"
...
3. カラム名の更新（Supabaseクエリの .eq, .select, .order 等の中）:
   - "league_id" → "league_id"
   - "round_id" → "round_id"（matches, team_entries, participants 等のFK）
   - "league_config" → "league_config"
   - "format" → "format"
   - "round_order" → "round_order"
   - FK結合の "!series_" → "!leagues_"（例: profiles!leagues_organizer_id_fkey → profiles!leagues_organizer_id_fkey）
...
- FK名の "leagues_organizer_id_fkey" → "leagues_organizer_id_fkey" 等はSupabaseが自動生成する名前なのでクエリ内の文字列として更新
'
fi
